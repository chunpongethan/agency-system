...
def test_policy_dates_and_arbitrary_integer_notional(client):
    ids = client._ids
    h = auth(client, "ADM")
    # 123457 is not a round thousand — any integer notional must be accepted.
    r = _book(client, h, ids, ids["ins"], notional="123457",
              policy_date="2026-08-20", effective_date="2026-09-01")
    assert r.status_code == 200, r.text
    body = r.json()
    assert Decimal(body["notional"]) == Decimal("123457")
    assert body["policy_date"] == "2026-08-20"
    assert body["effective_date"] == "2026-09-01"
